	...
	: IMapGenerator
{
	public Map Generate(int width, int height)
	{
		...
		for (var i = 0; i < 10; i++)
		{
			...
			do
			{
				...
				|| map.Tiles[x - 1, y].Character == '#'
				|| map.Tiles[x + 1, y].Character == '#'
				|| map.Tiles[x, y - 1].Character == '#'
				|| map.Tiles[x, y + 1].Character == '#'
	...
	private void BuildVerticalWall(Map map, int xWallPos)
	{
		...
		{
			map.Tiles[xWallPos, y].Character = '#';
		}
	}

	private void BuildVerticalWall(Map map, int xWallPos, int yDoorPos)
	{
		for (var y = yDoorPos + 1; y < map.Height && map.Tiles[xWallPos, y].Character == '.'; y++)
		{
			if (y == yDoorPos)
			{
				map.Tiles[xWallPos, y].Character = '+';
		...
		for (var y = yDoorPos - 1; y >= 0 && map.Tiles[xWallPos, y].Character == '.'; y--)
		{
			if (y == yDoorPos)
			{
				map.Tiles[xWallPos, y].Character = '+';
	...
	private void BuildHorizontalWall(Map map, int yWallPos)
	{
		...
		{
			map.Tiles[x, yWallPos].Character = '#';
		}
	}

	private void BuildHorizontalWall(Map map, int yWallPos, int xDoorPos)
	{
		for (var x = xDoorPos + 1; x < map.Width && map.Tiles[x, yWallPos].Character == '.'; x++)
		{
			if (x == xDoorPos)
			{
				map.Tiles[x, yWallPos].Character = '+';
